...
namespace CorreiosAutomation.Pages
{
    public class CorreiosHomePage
    {
        ...
        // URL da busca de CEP
        private const string UrlCep = "https://buscacepinter.correios.com.br/app/cep/index.php";
        ...
        {
            _driver.Navigate().GoToUrl(UrlCep);
        ...
        public string ObterEnderecoEncontrado()
        {
            try
            {
                var elementos = _driver.FindElements(ResultadoEnderecoXpath);
                var endereco = string.Join(" ", elementos.Select(e => e.Text).Where(s => !string.IsNullOrWhiteSpace(s)));
                return endereco;
            }
            catch
            {
